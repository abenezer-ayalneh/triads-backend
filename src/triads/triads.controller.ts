...
export class TriadsController {
	...
	async checkSolution(@Query() checkSolutionDto: CheckSolutionDto) {
		const matchedTriad = await this.triadsService.getMatchedTriads(checkSolutionDto.cues)
	...
	async checkAnswer(@Query() checkAnswerDto: CheckAnswerDto) {
		const matchedTriad = await this.triadsService.getMatchedTriads(checkAnswerDto.cues)
